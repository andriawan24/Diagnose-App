...
class DiagnoseController extends Controller
{
    ...
    public function symptoms(Request $request) {
        try {
            ...
            if ($request->search) {
                $symptoms->where('name', 'like', '%' . $request->search . '%');
                $symptoms->orWhere('name', 'like', '%' . $request->search . '%');
            }
    ...
    public function getHistories(Request $request) {
        try {
            ...
            if ($sort) {
                switch ($sort) {
                    case 'longest':
                        $consultations->orderBy('created_at');
                        break;
                    case 'by_name':
                        $consultations->orderBy('name');
                        break;
                    default:
            ...
            ], 'Get Consultation History success');
        } catch (Exception $e) {
            return ResponseFormatter::error([
                'message' => 'Something went wrong',
                'errors' => $e->getMessage()
            ], 'Get Consultation History failed', 400);
        }
    }
}
